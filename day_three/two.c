...
int is_valid_do_format(const char *str) {
    const char *p = str;

    // Check for "mul(" prefix
    if (strncmp(p, "do()", 4) != 0) return 0;
    p += 4;
...
void find_mul_patterns(const char *filename) {
    ...
    while ((c = fgetc(file)) != EOF) {
        if (c == 'm') {
            ...
            if (c == ')') {
                buffer[pos] = '\0';
                if ((is_valid_mul_format(buffer)) && (enabled == 1) ) {
                    result += extract_and_multiply(buffer);
                } else if ((enabled == 0) && (strncmp(buffer, "do()", 4) == 0)) {
                    enabled = 1;
                } else if ((enabled == 1) && (strncmp(buffer, "don't()", 7) == 0)) {
                    enabled = 0;
                }
                pos = 0;
                memset(buffer, 0, sizeof(buffer));
